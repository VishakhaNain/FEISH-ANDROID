...
import com.app.feish.application.Patient.PuchaseDoctorPlan;
...
public class PayUMoneyActivityforPurchasePlan extends AppCompatActivity {

    /**
     * Adding WebView as setContentView
     */
    WebView webView;
   drplanmodel drplanmodels;
...
*/
        ...
        if (bundle != null) {
                drplanmodels=(drplanmodel) bundle.getSerializable("plandata");
                doc_id=bundle.getString("doc_id");
                service_id=bundle.getString("service_id");

            mFirstName = PatientDashboard.contactService_getDetails.getData().getFirstName()+" "+PatientDashboard.contactService_getDetails.getData().getLastName();;
            mEmailId = PatientDashboard.contactService_getDetails.getData().getIs_active().toString();
            fee=drplanmodels.getPlanprice();
            //fee="1";
            mAmount = Double.parseDouble(fee);
            mPhone = PatientDashboard.contactService_getDetails.getData().getMobile();
            mId = 105;
            isFromOrder = true;
        ...
        webView.addJavascriptInterface(new PayUJavaScriptInterface(PayUMoneyActivityforPurchasePlan.this), "PayUMoney");
    ...
    private void onPressingBack() {
        ...
        if(isFromOrder)
            intent = new Intent(PayUMoneyActivityforPurchasePlan.this, PuchaseDoctorPlan.class);
        else
            intent = new Intent(PayUMoneyActivityforPurchasePlan.this, PuchaseDoctorPlan.class);

        intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);

        AlertDialog.Builder alertDialog = new AlertDialog.Builder(PayUMoneyActivityforPurchasePlan.this);
    ...
    public class PayUJavaScriptInterface {
        ...
        public void success(long id, final String paymentId) {
            mHandler.post(new Runnable() {
                ...
                public void run() {
                    mHandler = null;
                    Toast.makeText(PayUMoneyActivityforPurchasePlan.this, "Payment Successfully.", Toast.LENGTH_SHORT).show();
                    update("Card",0);
                }
            });
        }
    ...
    private Request updateappodetail(String paymentmpde) {
        ...
        try{
            postdata.put("user_id",Prefhelper.getInstance(activity).getUserid());
            postdata.put("planid",drplanmodels.getId());
            postdata.put("service_id",service_id);
            postdata.put("doctor_id",doc_id);
            postdata.put("name",drplanmodels.getPlanname());
            postdata.put("price",drplanmodels.getPlanprice());
            postdata.put("plan_details",drplanmodels.getPlandesc());
            postdata.put("valid_visits",Integer.parseInt(drplanmodels.getPlanvalidvisit()));
            postdata.put("validity",drplanmodels.getPlanvalidity());
            postdata.put("transactionid",mTXNId);
        ...
        return new Request.Builder()
                ...
                .url(ApiUtils.BASE_URL+"addPatientplan")
                .post(body)
                .build();
