...
import com.app.feish.application.doctor.DoctorPackageDetail;
...
public class PayUMoneyActivityforDoctorPackage extends AppCompatActivity {
    ...
    WebView webView;
    int planid;
    String currentdate="",increaseday="";
    /**
...
*/
        ...
        if (bundle != null) { 
            ...
            currentdate=bundle.getString("puchase_sdate");
            increaseday=bundle.getString("puchase_edate");
            mFirstName = DoctorDashboard.contactService_getDetails.getData().getFirstName()+" "+DoctorDashboard.contactService_getDetails.getData().getLastName();;
            mEmailId = DoctorDashboard.contactService_getDetails.getData().getIs_active().toString();
            //fee="1";
            mAmount = Double.parseDouble(fee);
            mPhone = DoctorDashboard.contactService_getDetails.getData().getMobile();
            mId = 105;
            isFromOrder = true;
         ...
         * Final Action URL...
         */
//            mAction = mBaseURL.concat("/_payment"); //Production
        ...
        webView.addJavascriptInterface(new PayUJavaScriptInterface(PayUMoneyActivityforDoctorPackage.this), "PayUMoney");
    ...
    private void onPressingBack() {
        ...
        if(isFromOrder)
            intent = new Intent(PayUMoneyActivityforDoctorPackage.this, DoctorPackageDetail.class);
        else
            intent = new Intent(PayUMoneyActivityforDoctorPackage.this, DoctorPackageDetail.class);

        intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);

        AlertDialog.Builder alertDialog = new AlertDialog.Builder(PayUMoneyActivityforDoctorPackage.this);
    ...
    public class PayUJavaScriptInterface {
        ...
        public void success(long id, final String paymentId) {
            mHandler.post(new Runnable() {
                ...
                public void run() {
                    mHandler = null;
                    Toast.makeText(PayUMoneyActivityforDoctorPackage.this, "Payment Successfully.", Toast.LENGTH_SHORT).show();
                   update();
                }
            });
        }
    ...
    private Request updateappodetail() {
        ...
        try{
            postdata.put("doc_id", Prefhelper.getInstance(activity).getUserid());
            postdata.put("planid",planid);
            postdata.put("puchase_sdate",currentdate);
            postdata.put("puchase_edate",increaseday);
            postdata.put("amount",fee);
